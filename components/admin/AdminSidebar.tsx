...
import { LayoutDashboard, Building2, Settings, Home, Mail, ChevronRight, BarChart3 } from "lucide-react";
...
export function AdminSidebar({ pendingCount = 0, unreadMessages = 0 }: AdminSidebarProps) {
    ...
    return (
        <aside className="w-72 bg-[#070c1a] text-slate-300 flex flex-col pt-12 rounded-r-3xl shadow-2xl overflow-hidden relative">
            ...
            <div className="px-8 mb-12 relative z-10 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-amber-500 flex items-center justify-center text-black shadow-lg">
            ...
            <nav className="flex-1 w-full px-4 space-y-1 relative z-10">
                {navItems.map((item) => {
            ...
            <div className="w-full p-4 mt-auto relative z-10">
                <Link
                    href="/"
                    className="flex items-center gap-3 px-4 py-4 rounded-2xl text-sm font-medium text-slate-500 hover:text-white hover:bg-slate-800/50 transition-all group"
                >
